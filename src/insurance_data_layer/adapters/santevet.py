...
class SantevetAdapter(SourceAdapter):
    ...
    def adapt(self, raw: dict[str, Any], raw_file: str) -> CanonicalRecord:
        ...
        rules = [
            ...
            ExcessRule(
                amount=Money(
                    amount=output["excess_amount"], currency=output["premium_currency"]
                ),
                scope="unknown",
            ...
            if amount is not None and amount > 0:
                rules.append(
                    LimitRule(
                        ...
                        amount=Money(amount=amount, currency=output["premium_currency"]),
                        scope="policy_year",
                    )
                )
        ...
        issues = [
            QualityIssue(
                ...
                message="The source does not prove whether excess is annual, per claim, or per incident.",
            ),
            QualityIssue(
                code="untrusted_source_pet_id",
                field="output.pet_id",
                message="Source pet_id varies across plan options; a deterministic request key was generated.",
                severity="info",
            ),
        ]
        ...
        return CanonicalRecord(
            ...
            provider=Provider(code="santevet", name=output["provider_name"]),
            product=Product(
                code="santevet_pet_health_fr",
                name="Pet health insurance",
                insurance_type=InsuranceType.PET_HEALTH,
                market_country=output.get("insurance_metadata", {}).get("country_code"),
            ),
            plan=Plan(code=plan_code, name=output["package_name"], version_key=version_key),
            quote_request=QuoteRequest(
                request_key=request_key,
                pet=PetRisk(
                    species={"CHAT": "cat", "CHIEN": "dog"}.get(
                        source_input["species"], source_input["species"].lower()
                    ),
                ...
                applicant=ApplicantRisk(postcode=source_input.get("city")),
